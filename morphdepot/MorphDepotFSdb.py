...
from morphdepot.models import resources
...
class FusqlDb(object):
    ...
    @fusqlogger.log()
    def get_scientist_captions(self):
        """
        Return captions of all scientists.
        """
        captions = []
        for scientist in self.session.query(resources.Scientist).all():
            print scientist.caption
            captions.append(scientist.caption)
        return captions

    @fusqlogger.log()
    def add_scientist(self, caption):
        """
        Return captions of all scientists.
        """
        names = resources.Scientist.caption2name(caption)
        scientist = resources.Scientist(
            first_name=names['first_name'],
            last_name=names['last_name']
        )
        self.session.add(scientist)
        self.session.commit()
